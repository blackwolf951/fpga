...
// TOP Q9(b). light bits are {L1,L2,L3,L4,L5,L6}; L7-L12 are off.
...
// [逐行教學 L021][語法] 使用阻塞賦值；在程序區塊內依順序立即更新，常用於組合計算/function；`6'b001100` = 6-bit 二進位常數；`6'b010100` = 6-bit
// [逐行教學 L021][語法] 二進位常數；`6'b100001` = 6-bit 二進位常數。
// [逐行教學 L021][用途] 硬體上通常形成解碼器/MUX，用狀態、按鍵或索引選擇不同輸出；完成目前 case 解碼邏輯；提供未列舉情況的安全輸出/預設動作；把 `else` 子模組放進目前模組並與內部訊號連線；更新
// [逐行教學 L021][用途] `lights`，下一值由 `6'b001100` 計算；更新 `lights`，下一值由 `6'b010100` 計算；更新 `lights`，下一值由 `6'b100001` 計算。
  else case(mode) 0:lights=6'b001100;1:lights=6'b010100;default:lights=6'b100001; endcase
...
// [逐行教學 L025][語法] `if (mode==0)` 是條件分支：條件為 1 才執行後續敘述；`if (mode==1)` 是條件分支：條件為 1 才執行後續敘述；`else if` 只在前面 if
// [逐行教學 L025][語法] 不成立時繼續測試下一個條件；`frame = ...` 使用阻塞賦值；在程序區塊內依順序立即更新，常用於組合計算/function；`color = ...`
// [逐行教學 L025][語法] 使用阻塞賦值；在程序區塊內依順序立即更新，常用於組合計算/function；`show = ...` 使用阻塞賦值；在程序區塊內依順序立即更新，常用於組合計算/function；`end else
// [逐行教學 L025][語法] begin` 先結束前一分支，再開始 else 的多敘述分支；`==` 是相等比較，結果為 1 或 0。
// [逐行教學 L025][用途] 檢查 `mode==0` 以決定資料/狀態要走哪一條路徑；檢查 `mode==1` 以決定資料/狀態要走哪一條路徑；更新 `frame`（動畫影格），下一值由 `phase` 計算；更新
// [逐行教學 L025][用途] `color`為 0/關閉或初始值；更新 `show`，下一值由 `1` 計算；更新 `frame`（動畫影格），下一值由 `4+phase` 計算；更新 `color`，下一值由 `2` 計算；更新
// [逐行教學 L025][用途] `frame`（動畫影格）為 0/關閉或初始值；更新 `color`，下一值由 `1` 計算。
  else begin if(mode==0) begin frame=phase;color=0;show=1;end else if(mode==1) begin frame=4+phase;color=2;show=1;end else begin frame=0;color=1;show=1;end end
...
// [逐行教學 L032][用途] 檢查 `!day_night` 以決定資料/狀態要走哪一條路徑。
  if(!day_night) begin
// [逐行教學 L033][語法] `digit = ...` 使用阻塞賦值；在程序區塊內依順序立即更新，常用於組合計算/function；`4'hF` = 4-bit 十六進位常數。
// [逐行教學 L033][用途] 更新 `digit`（目前七段顯示數字），下一值由 `4'hF` 計算。
   digit=4'hF;
// [逐行教學 L034][語法] `else` 是前面條件都不成立時執行的備援分支；`end else begin` 先結束前一分支，再開始 else 的多敘述分支。
// [逐行教學 L034][用途] 完成目前模組資料路徑或控制流程中的一個步驟。
  end else begin
// [逐行教學 L035][語法] `case(seg7_sel)` 是多路選擇語法，依選擇值匹配各個 `標籤:`。
// [逐行教學 L035][用途] 硬體上通常形成解碼器/MUX，用狀態、按鍵或索引選擇不同輸出。
   case(seg7_sel)
// [逐行教學 L036][語法] `3'b000` 是 case item 標籤；case 值匹配時執行冒號後動作；`digit = ...`
// [逐行教學 L036][語法] 使用阻塞賦值；在程序區塊內依順序立即更新，常用於組合計算/function；`3'b000` = 3-bit 二進位常數；`3'b001` = 3-bit 二進位常數；`4'hF` = 4-bit
// [逐行教學 L036][語法] 十六進位常數。
// [逐行教學 L036][用途] 定義這個選擇值對應的輸出或下一狀態；更新 `digit`（目前七段顯示數字），下一值由 `4'hF` 計算。
    3'b000,3'b001: digit=4'hF;
// [逐行教學 L037][語法] `3'b010` 是 case item 標籤；case 值匹配時執行冒號後動作；`digit = ...`
// [逐行教學 L037][語法] 使用阻塞賦值；在程序區塊內依順序立即更新，常用於組合計算/function；`3'b010` = 3-bit 二進位常數；`3'b011` = 3-bit 二進位常數；`4'd0` = 4-bit
// [逐行教學 L037][語法] 十進位常數。
// [逐行教學 L037][用途] 定義這個選擇值對應的輸出或下一狀態；更新 `digit`（目前七段顯示數字），下一值由 `4'd0` 計算。
    3'b010,3'b011: digit=4'd0;
// [逐行教學 L038][語法] `3'b100` 是 case item 標籤；case 值匹配時執行冒號後動作；`digit = ...`
// [逐行教學 L038][語法] 使用阻塞賦值；在程序區塊內依順序立即更新，常用於組合計算/function；`3'b100` = 3-bit 二進位常數。
// [逐行教學 L038][用途] 定義這個選擇值對應的輸出或下一狀態；更新 `digit`（目前七段顯示數字），下一值由 `red_count/10` 計算。
    3'b100: digit=red_count/10;
// [逐行教學 L039][語法] `default:` 是 case 沒有其他標籤匹配時使用的預設分支；`digit = ...` 使用阻塞賦值；在程序區塊內依順序立即更新，常用於組合計算/function。
// [逐行教學 L039][用途] 提供未列舉情況的安全輸出/預設動作；更新 `digit`（目前七段顯示數字），下一值由 `red_count-(red_count/10)*10` 計算。
...
// [逐行教學 L043][語法] `bcd_to_seg7 USD(...)` 是子模組實例化；`USD` 是這一顆硬體實例名稱；`.port(signal)` 是具名 port mapping。
// [逐行教學 L043][用途] 七段顯示器解碼器：把 4-bit 數字/代碼轉成 a~g 七段亮滅資料。。
 bcd_to_seg7 USD(.bcd_in(digit),.seg7(seg7_out));
// [逐行教學 L044][語法] `endmodule` 與前面的 `module` 配對，結束模組定義。
// [逐行教學 L044][用途] 結束 `traffic_top` 的 RTL 描述。
endmodule
