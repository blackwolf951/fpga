...
// [逐行教學 L008][用途] 本段宣告：d5, d4, d3, d2, d1, d0。
...
// [逐行教學 L013][用途] 計時/倒數核心：依時脈更新數字資料。。
    clock U_CNT(.clk(clk_1hz),.reset(reset),.tick_1hz(1'b1),.enable(enable),
// [逐行教學 L014][語法] 這是一行 Verilog RTL 敘述；它延續目前區塊的宣告、控制或資料處理。
// [逐行教學 L014][用途] 完成目前模組資料路徑或控制流程中的一個步驟。
                .count5(d5),.count4(d4),.count3(d3),.count2(d2),.count1(d1),.count0(d0),.done(carry));
// [逐行教學 L015][語法] `seg7_select U_SEL(...)` 是子模組實例化；`U_SEL` 是這一顆硬體實例名稱；`.port(signal)` 是具名 port mapping；`1'b1` = 1-bit
// [逐行教學 L015][語法] 二進位常數。
// [逐行教學 L015][用途] 七段顯示器位選掃描器：快速輪流選擇多位七段顯示器。。
    seg7_select U_SEL(.clk(clk_scan),.reset(reset),.tick_scan(1'b1),.seg7_sel(seg7_sel));
// [逐行教學 L016][語法] `always @(*)` 自動把區塊內讀到的訊號加入敏感度表，通常描述組合邏輯；同一行的 `begin` 開始一個多敘述區塊。
// [逐行教學 L016][用途] 任一輸入改變就重新計算本區塊；所有路徑都有賦值時不需要記憶狀態。
    always @(*) begin
// [逐行教學 L017][語法] `case(seg7_sel)` 是多路選擇語法，依選擇值匹配各個 `標籤:`。
// [逐行教學 L017][用途] 硬體上通常形成解碼器/MUX，用狀態、按鍵或索引選擇不同輸出。
        case(seg7_sel)
// [逐行教學 L018][語法] `3'b000` 是 case item 標籤；case 值匹配時執行冒號後動作；`mux_digit = ...`
// [逐行教學 L018][語法] 使用阻塞賦值；在程序區塊內依順序立即更新，常用於組合計算/function；`3'b000` = 3-bit 二進位常數；`3'b001` = 3-bit 二進位常數；`3'b010` = 3-bit
// [逐行教學 L018][語法] 二進位常數。
// [逐行教學 L018][用途] 定義這個選擇值對應的輸出或下一狀態；更新 `mux_digit`，下一值由 `d5` 計算；更新 `mux_digit`，下一值由 `d4` 計算；更新 `mux_digit`，下一值由 `d3`
// [逐行教學 L018][用途] 計算。
            3'b000:mux_digit=d5; 3'b001:mux_digit=d4; 3'b010:mux_digit=d3;
// [逐行教學 L019][語法] `default:` 是 case 沒有其他標籤匹配時使用的預設分支；`3'b011` 是 case item 標籤；case 值匹配時執行冒號後動作；`mux_digit = ...`
// [逐行教學 L019][語法] 使用阻塞賦值；在程序區塊內依順序立即更新，常用於組合計算/function；`3'b011` = 3-bit 二進位常數；`3'b100` = 3-bit 二進位常數。
// [逐行教學 L019][用途] 提供未列舉情況的安全輸出/預設動作；定義這個選擇值對應的輸出或下一狀態；更新 `mux_digit`，下一值由 `d2` 計算；更新 `mux_digit`，下一值由 `d1` 計算；更新
// [逐行教學 L019][用途] `mux_digit`，下一值由 `d0` 計算。
            3'b011:mux_digit=d2; 3'b100:mux_digit=d1; default:mux_digit=d0;
...
// [逐行教學 L022][用途] 保留原作者對題目、接線、狀態或設計意圖的文字說明。
    // HH.MM.SS: dot after hour ones and minute ones.
// [逐行教學 L023][語法] `assign 左式 = 右式;` 是 continuous assignment；這裡的 `=` 屬連續指定，不是 always 內的阻塞賦值；`3'b001` = 3-bit
// [逐行教學 L023][語法] 二進位常數；`3'b011` = 3-bit 二進位常數；`==` 是相等比較，結果為 1 或 0；`||` 是邏輯 OR：任一條件成立即為真。
// [逐行教學 L023][用途] 更新 `dpt_out`，下一值由 `(seg7_sel==3'b001 || seg7_sel==3'b011)` 計算；右式一變化就立即重新驅動左側 wire。
    assign dpt_out = (seg7_sel==3'b001 || seg7_sel==3'b011);
// [逐行教學 L024][語法] `bcd_to_seg7 U_DEC(...)` 是子模組實例化；`U_DEC` 是這一顆硬體實例名稱；`.port(signal)` 是具名 port mapping。
// [逐行教學 L024][用途] 七段顯示器解碼器：把 4-bit 數字/代碼轉成 a~g 七段亮滅資料。。
    bcd_to_seg7 U_DEC(.bcd_in(mux_digit),.seg7(seg7_out));
// [逐行教學 L025][語法] `endmodule` 與前面的 `module` 配對，結束模組定義。
// [逐行教學 L025][用途] 結束 `clock_top` 的 RTL 描述。
endmodule
