...
// TOP Q7(b). New key enters the LEFT side; old digits shift RIGHT.
...
// [逐行教學 L011][用途] `last_code`，下一值由 `key_code` 計算。
  else if(key_event) begin display_code<={key_code,display_code[23:4]};last_code<=key_code;end
...
// [逐行教學 L016][語法] `chinese_num_rom UROM(...)` 是子模組實例化；`UROM` 是這一顆硬體實例名稱；`.port(signal)` 是具名 port mapping。
// [逐行教學 L016][用途] 中文字/數字字型 ROM。。
 chinese_num_rom UROM(.code(last_code),.row_idx(row_idx),.data(matrix_pixels));
